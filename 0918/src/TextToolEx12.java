		...
		btn[n++].addActionListener(new ActionListener() { // 패턴제거
			public void actionPerformed(ActionEvent ae) {
				...
				for (int i = 0; sc.hasNextLine(); i++) {
					...
					// 1-1. 읽은 문자뎔 중 패턴식 문자열을 제외하고 군더더기는 삭제
					String tokenString = line.split(":")[1];
					
					// 2. 각 라인을 pattern에 맞게 매칭시킨다.(Pattern클래스의 matcher() 사용)
					Matcher matcher = p.matcher(tokenString);
					
					// 3. pattern에 매칭되는 데이터를 구분자와 함께 sb에 저장한다.
					if (matcher.matches()) { // 3-1. 패턴과 일치하는 문자열을 찾았다면.
						// 3-2. String split 메서드를 사용하여 문자열 배열에 담는다
